import numpy as np
import pygame
import sys #As an exit function
pygame.init()
...
screen = pygame.display.set_mode((WIDTH,HEIGHT)) #for screen display
pygame.display.set_caption("Tic Tac Toe") #Title of screen
screen.fill(BLACK) # fill the screen with black colour

board = np.zeros((BOARD_ROWS,BOARD_COLS))  #We just define the board not drawing it yet
...
def draw_figures(color = WHITE):
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLS):
            if (board[row][col]==2):
                pygame.draw.circle(screen,color,(int(SQUARE_SIZE*col + SQUARE_SIZE//2),int(SQUARE_SIZE*row+SQUARE_SIZE//2)),CIRCLE_RADIUS,CIRCLE_LINE_WIDTH)
            if(board[row][col]==1):
